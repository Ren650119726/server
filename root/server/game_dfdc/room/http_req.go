...
func asyn_addMoney(addr_url, unique string, num int64, roomID int32, desc string, back func(backunique string, backmoney int64, bwType int32), errback func()) { // dfdc
	go func() {
		...
		resp, err := http.PostForm(addr_url,
			send)
		log.Infof("请求下注:%v, addr:%v", send, addr_url)
		...
		log.Infof("多福多财请求下注，平台返回:%v", string(body))
