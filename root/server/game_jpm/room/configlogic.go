...
func (self *Room) LoadConfig()  {
	bets_conf := config.Get_configString("jpm_room",int(self.roomId),"Bet")
	self.bets = utils.SplitConf2ArrUInt64(bets_conf)

	self.basics = int64(config.Get_configInt("jpm_room",int(self.roomId),"JackpotBase"))
	self.jackpotRate = uint64(config.Get_configInt("jpm_room",int(self.roomId),"JackpotRole"))
	self.jackLimit = int64(config.Get_configInt("jpm_room",int(self.roomId),"JackpotBet"))
	...
	for _,id := range protomsg.Fruit1ID_value{
		...
		self.mapPictureNodes[int(id)] = &pictureNode{
			cfId:    int(id),
			cfOdd_2: config.Get_configInt("jpm_pattern",int(id),"Odds2"),
			cfOdd_3: config.Get_configInt("jpm_pattern",int(id),"Odds3"),
			cfOdd_4: config.Get_configInt("jpm_pattern",int(id),"Odds4"),
			cfOdd_5: config.Get_configInt("jpm_pattern",int(id),"Odds5"),
		}
	}

	self.lineConf = make([][5]int,10,10)
	conf := config.Get_config("jpm_lines")
	for id,_ := range conf{
		for i:=1;i <=5;i++{
			val := config.Get_configInt("jpm_lines",id,fmt.Sprintf("site%v",i))
			self.lineConf[id][i-1] = val-1
		}
	}
	self.mainWheel,self.freeWheel = initWheel(int64(config.Get_configInt("jpm_room",int(self.roomId),"Real")))
...
func initWheel(group int64) (main,free []*wheelNode ) {
	...
	conf := config.Get_config("jpm_real")
	for id,_ := range conf {
		if config.Get_configInt("jpm_real",id,"Group_id")  != int(group){
			continue
		}
		node := new(wheelNode)
		node.cfPosition = config.Get_configInt("jpm_real",id,"Site")
		if node.cfPosition > 0 {
			for i := 1; i <= 5; i++ {
				value := config.Get_configInt("jpm_real",id,fmt.Sprintf("Real%v",i))
				node.ids = append(node.ids, value)
			}
			if t := config.Get_configInt("jpm_real",id,"Type");t == 1{
...
return:
	...
	@ args[4] 获得大奖的数量
*/
func (self *Room) selectWheel(nodes []*wheelNode, betNum int64, isKill,test bool) ([]*protomsg.JPM_Result, []int32, int, int,int64, int64) {
	...
	if spcifity_2_count >= 3{
		freeCount = config.Get_configInt("jpm_pattern",2,fmt.Sprintf("Free%v",spcifity_2_count))
	}

	tmp := make([]*protomsg.JPM_Result, 0)
	sumOdds := 0
	reward := int64(0)
	bingocount := 0
	// 判断所有中奖线路
	for lid,line := range self.lineConf{
		...
		positions := make([]*protomsg.JPMPosition, 0)
		tempArr := []int{} // 中奖线图片组
		tempposs := []*protomsg.JPMPosition{}
		for _,pos := range line {
			x := pos % 3
			y := pos / 3
			id := b[x][y]
			tempArr = append(tempArr, id)
			tempposs = append(tempposs,&protomsg.JPMPosition{Px:int32(x),Py:int32(y)})
		}
		...
		for _,pos := range line {
			...
			x := pos % 3
			y := pos / 3
			positions = append(positions, &protomsg.JPMPosition{int32(x), int32(y)})
			ii++
		}
		...
		if bingo == 3 && count >= 3{
			...
			val := config.Get_configInt("jpm_pattern",3,fmt.Sprintf("Jackpot%v",count))
...
func (self *Room) win(arr []int,inpos []*protomsg.JPMPosition)  (count,bingo int){
	//判断是否中奖
	number := arr[0]
	count = 1
	cont := true
