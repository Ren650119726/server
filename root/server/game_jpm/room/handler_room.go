...
func (self *Room) JPMMSG_CS_START_JPM_REQ(actor int32, msg []byte, session int64) {
	...
	acc.LastBet = BetNum
	gameFun := func() {
		resluts := make([]*protomsg.JPM_Result, 0)
		pArr := make([]int32, 0)
		gainFreeCount := int(0)
		sumOdds := int64(0)
		reward := int64(0)
		...
		if acc.GetOSType() == 4 {
			self.owner.AddTimer(500, 1, func(dt int64) {
				platform.Asyn_addMoney(5, self.addr_url, acc.UnDevice, val, int32(self.roomId), "game_hongbao", "金品梅 中奖", nil, nil) //中奖
			})
		}
	...
	if !isFree {
		...
		if acc.GetOSType() == 4 {
			...
			platform.Asyn_addMoney(5, self.addr_url, acc.UnDevice, -int64(BetNum), int32(self.roomId), "game_hongbao", fmt.Sprintf("金瓶梅请求下注:%v", BetNum), back, errback)
		} else {
