...
// 大厅发送的玩家数据
func (self *lhd) SERVERMSG_HG_PLAYER_DATA_REQ(actor int32, msg []byte, session int64) {
...
// 玩家请求进入房间
func (self *lhd) LHDMSG_CS_ENTER_GAME_LHD_REQ(actor int32, data []byte, session int64) int32 {
	enterPB := packet.PBUnmarshal(data, &protomsg.ENTER_GAME_LHD_REQ{}).(*protomsg.ENTER_GAME_LHD_REQ)
