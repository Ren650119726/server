...
// 玩家离开
func (self *Room) LHDMSG_CS_LEAVE_GAME_LHD_REQ(actor int32, msg []byte, session int64) {
	enterPB := packet.PBUnmarshal(msg, &protomsg.LEAVE_GAME_LHD_REQ{}).(*protomsg.LEAVE_GAME_LHD_REQ)
	ret := uint32(1)
	if self.canleave(enterPB.GetAccountID()) {
		ret = 0
	}
	send_tools.Send2Account(protomsg.LHDMSG_SC_LEAVE_GAME_LHD_RES.UInt16(), &protomsg.LEAVE_GAME_LHD_RES{
		Ret:    ret,
		RoomID: self.roomId,
	}, session)
}
...
// 请求玩家列表
func (self *Room) LHDMSG_CS_PLAYERS_LHD_LIST_REQ(actor int32, msg []byte, session int64) {
	account.AccountMgr.GetAccountBySessionIDAssert(session)

	ret := &protomsg.PLAYERS_LHD_LIST_RES{}
	...
	send_tools.Send2Account(protomsg.LHDMSG_SC_PLAYERS_LHD_LIST_RES.UInt16(), ret, session)
}
