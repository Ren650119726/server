...
import (
	...
	"root/server/game_luckfruit/room"
...
func Newluckfruit() *luckfruit {
	return &luckfruit{}
}

func (self *luckfruit) Init(actor *core.Actor) bool {
	// 先处理脚本
	if core.ScriptDir != "" {
...
// 向hall注册
func (self *luckfruit) registerHall() {
	// 发送注册消息登记自身信息
...
func (self *luckfruit) StartService() {
	// 监听端口，客户端连接用
...
func (self *luckfruit) Stop() {

}

func (self *luckfruit) HandleMessage(actor int32, msg []byte, session int64) bool {
	if self.close {
		return true
	}
	...
	case protomsg.LUCKFRUITMSG_CS_ENTER_GAME_LUCKFRUIT_REQ.UInt16(): // 请求进入小玛利房间
		actor := self.LUCKFRUITMSG_CS_ENTER_GAME_LUCKFRUIT_REQ(actor, pack.ReadBytes(), session)
...
func (self *luckfruit) Old_MSGID_MAINTENANCE_NOTICE(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	if session != 0 {
