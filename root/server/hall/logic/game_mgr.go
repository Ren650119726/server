...
type (
	...
	gameMgr struct {
		...
		room_bonus  map[uint32]string    // 房间水池金额
		room_profit map[uint32]string    // 房间盈利
		savebounus  bool
	}
)
...
func newGameMgr() *gameMgr {
	hall := &gameMgr{
		...
		room_bonus:  make(map[uint32]string),
		room_profit: make(map[uint32]string),
	}
...
func (self *gameMgr) GetBaseInfo(roomID uint32) (minMoney uint64, t uint32, order uint32, bet []uint64) {
	...
	case common.EGameTypeJPM:
		minMoney = uint64(config.Get_configInt("jpm_room", int(roomID), "GlodNeed"))
		t = uint32(config.Get_configInt("jpm_room", int(roomID), "Type"))
		betstr := config.Get_configString("jpm_room", int(roomID), "Bet")
		bet = utils.SplitConf2ArrUInt64(betstr)
	...
	case common.EGameTypeRED2BLACK:
		minMoney = uint64(config.Get_configInt("red2black_room", int(roomID), "GlodNeed"))
		t = uint32(config.Get_configInt("red2black_room", int(roomID), "Type"))
		betstr := config.Get_configString("red2black_room", int(roomID), "Bet")
		bet = utils.SplitConf2ArrUInt64(betstr)
		order = uint32(config.Get_configInt("red2black_room", int(roomID), "Order"))
		return
	case common.EGameTypeLHD:
		minMoney = uint64(config.Get_configInt("lhd_room", int(roomID), "GlodNeed"))
		t = uint32(config.Get_configInt("lhd_room", int(roomID), "Type"))
		betstr := config.Get_configString("lhd_room", int(roomID), "Bet")
		bet = utils.SplitConf2ArrUInt64(betstr)
		order = uint32(config.Get_configInt("lhd_room", int(roomID), "Order"))
		return
	case common.EGameType777:
		minMoney = uint64(config.Get_configInt("777_room", int(roomID), "GlodNeed"))
		t = uint32(config.Get_configInt("777_room", int(roomID), "Type"))
		betstr := config.Get_configString("777_room", int(roomID), "Bet")
		bet = utils.SplitConf2ArrUInt64(betstr)
		order = uint32(config.Get_configInt("777_room", int(roomID), "Order"))
		return
	case common.EGameTypeHONGBAO:
		minMoney = uint64(config.Get_configInt("red_room", int(roomID), "GlodNeed"))
		t = uint32(config.Get_configInt("red_room", int(roomID), "Type"))
		betstr := config.Get_configString("red_room", int(roomID), "Bet")
		bet = utils.SplitConf2ArrUInt64(betstr)
		order = uint32(config.Get_configInt("red_room", int(roomID), "Order"))
		return
	default:
...
func (self *gameMgr) Save() {
	if self.savebounus {
		for roomid, bounusValue := range self.room_bonus {
	...
	for roomid, val := range self.room_profit {
		send_tools.Send2DB(inner.SERVERMSG_HD_SAVE_WATER_LINE.UInt16(), &inner.SAVE_WATER_LINE{
			RoomID:    roomid,
			WaterLine: val,
		})
	}
