...
func (self *Hall) MSG_CS_SAFEMONEY_OPERATE_REQ(actor int32, msg []byte, session int64) {
	...
	if pbMsg.GetOperate() == 1 { // 取钱
		...
		acc.AddMoney(int64(pbMsg.GetOperateMoney()), common.EOperateType_SAFE_MONEY_GET, 0)
		...
		acc.SafeMoney += pbMsg.GetOperateMoney()
		acc.AddMoney(-int64(pbMsg.GetOperateMoney()), common.EOperateType_SAFE_MONEY_SAVE, 0)
	} else {
