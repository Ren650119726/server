	...
	for _, cards := range all_t {
		...
		for i := 1; i <= lcards; i++ {
			...
			if continuous_count > 1 && reset_continuous {
				for ii := 2; ii <= continuous_count; ii++ {
					ret[tcard].continuous[ii]++
				}
				continuous_count = 1
			}
...
//	return ag.MajiangCalcHuType(self.cards.hand, self.cards.peng, self.cards.gang, 0)
//}
