...
func (self *Room) Old_MSGID_PANDA_AUDIENCE_LIST(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	accountId := pack.ReadUInt32()

	send := packet.NewPacket(nil)
	send.SetMsgID(protomsg.Old_MSGID_PANDA_AUDIENCE_LIST.UInt16())
...
// 请求坐下
func (self *Room) Old_MSGID_PANDA_SIT_DOWN(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	accountID := pack.ReadUInt32()

	send := packet.NewPacket(nil)
	send.SetMsgID(protomsg.Old_MSGID_PANDA_SIT_DOWN.UInt16())
	acc := account.AccountMgr.GetAccountByID(accountID)
	if acc == nil {
		send.WriteUInt8(1)
		send_tools.Send2Account(send.GetData(), session)
	...
	addplayer.SetMsgID(protomsg.Old_MSGID_PANDA_ADD_PLAYER.UInt16())
	...
	if acc.Profit > 0 && int64(acc.Games) < config.GetPublicConfig_Int64("PANDA_REWARD_QUIT_COUNT") {
		self.seats[index].safe_quit_timeout = utils.SecondTimeSince1970() + config.GetPublicConfig_Int64("PANDA_SAVE_QUIT_TIME")
	}
	...
	t := int(utils.SecondTimeSince1970() + config.GetPublicConfig_Int64("PANDA_READY_TIME"))
	// 如果人满，没有准备的开始倒计时
	...
	if send_wating_time {
		...
		s.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_UPDATE_WATING_TIME.UInt16())
		s.WriteInt64(int64(utils.MilliSecondTimeSince1970() + config.GetPublicConfig_Int64("PANDA_READY_TIME")*1000))
...
func (self *Room) Old_MSGID_ENTER_GAME(actor int32, msg []byte, session int64) {
	...
	if seati == -1 {
		if acc.AutoSitDown == 1 {
			...
			sitdown.SetMsgID(protomsg.Old_MSGID_PANDA_SIT_DOWN.UInt16())
			...
			log.Debugf("玩家 %v name %v 自动坐下", acc.AccountId, acc.Name)
...
// 请求盈利
func (self *Room) Old_MSGID_PANDA_PROFIT(actor int32, msg []byte, session int64) {
	recv := packet.NewPacket(msg)
	accid := recv.ReadUInt32()
	acc := account.AccountMgr.GetAccountByID(accid)

	send := packet.NewPacket(nil)
	send.SetMsgID(protomsg.Old_MSGID_PANDA_PROFIT.UInt16())
	send.WriteUInt32(uint32(accid))
	send.WriteInt64(acc.Profit - acc.ExtractBoun)
	send.WriteUInt32(uint32(acc.Games))
	send.WriteUInt32(uint32(config.GetPublicConfig_Int64("PANDA_PENALTY_RATIO")))
	send_tools.Send2Account(send.GetData(), session)

}

// 请求盈利
func (self *Room) Old_MSGID_PANDA_PRESON_INFO(actor int32, msg []byte, session int64) {
	send := packet.NewPacket(nil)
	tempCount := uint16(0)
	temp := packet.NewPacket(nil)
	...
	send.SetMsgID(protomsg.Old_MSGID_PANDA_PRESON_INFO.UInt16())
	send_tools.Send2Account(send.GetData(), session)
}

// 请求取消托管
func (self *Room) Old_MSGID_PANDA_GAME_STRUSATEESHIP_CANCEL(actor int32, msg []byte, session int64) {
	recv := packet.NewPacket(msg)
	acc := recv.ReadUInt32()
...
// 历史中奖记录
func (self *Room) PANDA_GAME_REWARD_HISTORY(actor int32, msg []byte, session int64) {
	bonush := RoomMgr.Bonus_h[uint32(self.GetParamInt(0))]
	if bonush == nil {
		return
	}
	send := packet.NewPacket(nil)
	send.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_REWARD_HISTORY.UInt16())
