...
func (self *Room) Init(actor *core.Actor) bool {
	strname := config.GetPublicConfig_String("DGK_REWARD_NAMES")
	self.reward_conf = utils.SplitConf2Mapis(strname)
	self.owner = actor

	str := config.GetPublicConfig_String("DGK_HU_FAN")
	self.hu_fan = utils.SplitConf2ArrInt32(str, ",")
	if len(self.hu_fan) != 12 {
		log.Errorf("配置不12个胡的番 :%v", len(self.hu_fan))
		return false
	}

	str = config.GetPublicConfig_String("DGK_EXTRA_FAN")
	self.extra_fan = utils.SplitConf2ArrInt32(str, ",")
	if len(self.extra_fan) != 12 {
		log.Errorf("配置不12个额外的番 :%v", len(self.extra_fan))
		return false
	}
...
func (self *Room) GetBounsType() int {
	conf_arr := config.GetPublicConfig_Slice("DGK_REWARD_POOL")
...
func (self *Room) HandleMessage(actor int32, msg []byte, session int64) bool {
	...
	case protomsg.Old_MSGID_DGK_AUDIENCE_LIST.UInt16(): // 请求玩家列表
		self.Old_MSGID_DGK_AUDIENCE_LIST(actor, msg, session)
	...
	case protomsg.Old_MSGID_DGK_SIT_DOWN.UInt16(): // 观众请求坐下
		self.Old_MSGID_DGK_SIT_DOWN(actor, msg, session)
	case protomsg.Old_MSGID_DGK_PRESON_INFO.UInt16(): // 战绩
		self.Old_MSGID_DGK_PRESON_INFO(actor, msg, session)
	case protomsg.Old_MSGID_DGK_GAME_STRUSATEESHIP_CANCEL.UInt16(): // 取消托管
		self.Old_MSGID_DGK_GAME_STRUSATEESHIP_CANCEL(actor, msg, session)
	case protomsg.Old_MSGID_DGK_PROFIT.UInt16():
		self.Old_MSGID_DGK_PROFIT(actor, msg, session)
	case protomsg.Old_MSGID_DGK_GAME_REWARD_HISTORY.UInt16():
		self.DGK_GAME_REWARD_HISTORY(actor, msg, session)
...
func (self *Room) sitDown(accountId uint32) int {
	...
	self.seats[index] = &GamePlayer{
		...
		time_of_join: -1, //utils.SecondTimeSince1970() + config.GetPublicConfig_Int64("DGK_READY_TIME"),
		cards:        NewCardGroup(),
	}
...
func (self *Room) enterRoom(accountId uint32) {
	...
	if inheritAcc, exit := self.inherits[accountId]; exit {
	...
	if acc.Robot == 0 {
		log.Infof(colorized.Cyan("-> In %v robot: Accid:%v Name:%v rmb:%v %v roomid:%v games:%v"), utils.DateString(), acc.AccountId, acc.Name, acc.GetMoney(), types.ERoomStatus(self.status.State()).String(), self.roomId, acc.Games)
	} else {
...
func (self *Room) leaveRoom(accountId uint32, penalty bool) bool {
	...
	if penalty {
		if int64(acc.Games) < config.GetPublicConfig_Int64("DGK_REWARD_QUIT_COUNT") {
			if acc.Profit > 0 {
				s := self.status.Current().(IDGKStatus_universal)
				if !s.SaveQuit(accountId) {
					log.Infof("退出惩罚accid:%v, profit:%v fee:%v ", acc.AccountId, acc.Profit, acc.Fee)
					penalty_val := (acc.Profit - acc.Fee - acc.ExtractBoun) * config.GetPublicConfig_Int64("DGK_PENALTY_RATIO") / 100
...
func (self *Room) status_obj() IDGKStatus_universal {
	status := self.status.Current()
	check, ok := status.(IDGKStatus_universal)
	if !ok {
		log.Errorf("IDGKStatus_universal 请检查代码 current_status:%v", self.status.State())
		return nil
	}
...
func (self *Room) sendGameData(acc *account.Account) packet.IPacket {
	...
	send2acc.SetMsgID(protomsg.Old_MSGID_DGK_GAME_DATA.UInt16())
	...
	DGK_AUTO_CHECK_CREATE_ROOM_COUNT_3 := config.GetPublicConfig_Mapi("DGK_AUTO_CHECK_CREATE_ROOM_COUNT_3")
	send2acc.WriteUInt16(uint16(len(DGK_AUTO_CHECK_CREATE_ROOM_COUNT_3)))
	for nBet := range DGK_AUTO_CHECK_CREATE_ROOM_COUNT_3 {
...
func (self *Room) broadcast_count() {
	...
	send2other.SetMsgID(protomsg.Old_MSGID_DGK_UPDATE_AUDIENCE.UInt16())
...
func (self *Room) Disconnect(session int64) {
	...
	if index != -1 {
		...
		offline.SetMsgID(protomsg.Old_MSGID_DGK_OFFLINE.UInt16())
