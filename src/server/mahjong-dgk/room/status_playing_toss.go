...
func (self *toss) Enter(now int64) {
	...
	duration := config.GetPublicConfig_Int64("DGK_AFTER_DEAL_TIME") // 持续时间 秒
	...
	for index, player := range self.seats {
		...
		// 能不能碰
		if p := algorithm.CheckPeng(player.cards.hand, card); p != -1 && self.wanGang_qinHu != 2 && player.jiao == nil && !player.exclude_peng[card] {
		...
		if g := algorithm.CheckGang(player.cards.hand, card); g != -1 && (all_card_count > 0) && self.wanGang_qinHu != 2 {
			...
			if player.jiao != nil {
				...
				gang = append(gang, player.cards.gang...)
				gang = append(gang, []common.EMaJiangType{card, card, card, card})
				hand = append(hand, player.cards.hand[:g]...)
				hand = append(hand, player.cards.hand[g+3:]...)
				j := algorithm.Jiao_(hand, player.cards.peng, gang)
				if len(j) == 0 {
					self.track_log(colorized.Blue("---- 能换叫 杠:%v"), g)
					opt.gang = 0
				} else {
					self.gangjiao[index] = j
				}
			} else {
				self.track_log(colorized.Blue("---- 能 杠:%v"), g)
			}
		}

		// 能不能胡
		t, h := self.point_pao_check(lastPlayer, card, player)
		if et := player.exclude_hu; t > 1 && (et <= 0 || t > uint8(et)) {
		...
		send.SetMsgID(protomsg.Old_MSGID_DGK_GAME_OPTION.UInt16())
...
func (self *toss) Tick(now int64) {
	...
	if now >= self.timestamp {
		...
		for index := range self.players {
			...
			data.SetMsgID(protomsg.Old_MSGID_DGK_GAME_GUO_REQ.UInt16())
	...
	for index, op := range self.players {
		...
		if p.trusteeship == 1 {
			if op.hu == 1 {
				...
				auto_send := packet.NewPacket(nil)
				auto_send.SetMsgID(protomsg.Old_MSGID_DGK_GAME_GUO_REQ.UInt16())
...
func (self *toss) point_pao_check(fangpao_er *GamePlayer, pao common.EMaJiangType, hu_er *GamePlayer) (uint8, common.EMaJiangHu) {
	...
	sort.Sort(so)
	all_hu = so.All
	if self.push_count == 1 && hu_er.jiao != nil && self.deal_player == self.master {
		all_hu[0].HuType = common.HU_DI
	}
	hu := all_hu[0].HuType
	pao_index := self.deal_player
	...
	// 计算抓报叫 最大番数 -1
	if zhua_bao[pao_index] != 0 {
		total_fan += uint8(zhua_bao[pao_index]) - 1
	}
...
func (self *toss) point_pao(fangpao_er *GamePlayer, pao common.EMaJiangType, hu_er *GamePlayer, mutilp bool, rew packet.IPacket) {
	...
	if arr_rate[int32(all_hu[0].HuType)] != 0 {
		...
		hu_er.acc.AddMoney(int64(val), 0, common.EOperateType_DGK_REWARD)
	...
	fangpao_er.acc.AddMoney(-total_score, 0, common.EOperateType_DGK_HU)
	hu_er.acc.AddMoney(total_score, 0, common.EOperateType_DGK_HU)
	...
	for t, c := range all_hu[0].Extra {
		if t == common.EXTRA_GANGSHANGPAO && c == 1 && !mutilp {
			...
			fangpao_er.acc.AddMoney(-total, 0, common.EOperateType_DGK_GANG)
			hu_er.acc.AddMoney(total, 0, common.EOperateType_DGK_GANG)
	...
	self.track_log(colorized.Blue("---- 放炮的人:%v 玩家:%v 胡牌:%v 总番:%v 额外番:%v, 被抓报叫:%v 抓请:%v 座位:%v 赔付总金额:%v"),
		fangpao_er.acc.AccountId, hu_er.acc.AccountId, hu_er.hu, total_fan, all_hu[0].Extra, zhua_bao[self.deal_player], zq, self.deal_player, total_score)
...
func (self *toss) optdeal() {
	...
	if lhu > 1 {
		...
		multip_.SetMsgID(protomsg.Old_MSGID_DGK_GAME_MULTIP_HU.UInt16())
		...
		for i, v := range lastPlayer.gang_score_z {
			...
			tp := self.seats[i]
			tp.acc.AddMoney(v, 0, common.EOperateType_DGK_GANG)
			lastPlayer.acc.AddMoney(-v, 0, common.EOperateType_DGK_GANG)
		}
	...
	if lhu != 0 {
		...
		if opt_man == PENG { // 碰操作!!!!!!!!!!!!!!!!!
			...
			sendpeng.SetMsgID(protomsg.Old_MSGID_DGK_GAME_PENG_REQ.UInt16())
			...
			sendgang.SetMsgID(protomsg.Old_MSGID_DGK_GAME_GANG_REQ.UInt16())
...
func (self *toss) Handle(actor int32, msg []byte, session int64) bool {
	...
	case protomsg.Old_MSGID_DGK_GAME_PENG_REQ.UInt16(): // 碰
		self.Old_MSGID_DGK_GAME_PENG_REQ(actor, msg, session)
	case protomsg.Old_MSGID_DGK_GAME_GANG_REQ.UInt16(): // 杠
		self.Old_MSGID_DGK_GAME_GANG_REQ(actor, msg, session)
	case protomsg.Old_MSGID_DGK_GAME_HU_REQ.UInt16(): // 胡
...
// 过
func (self *toss) Old_MSGID_DGK_GAME_GUO_REQ(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
...
// 碰
func (self *toss) Old_MSGID_DGK_GAME_PENG_REQ(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	index := int(pack.ReadInt8()) - 1
	if index == -1 {
		log.Warnf("出错！%v", index)
...
// 杠
func (self *toss) Old_MSGID_DGK_GAME_GANG_REQ(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	index := int(pack.ReadInt8()) - 1
	if index == -1 {
		log.Warnf("出错！%v", index)
...
// 胡
func (self *toss) Old_MSGID_DGK_GAME_HU_REQ(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
