...
func (self *PANDA) Old_MSGID_CREATE_ROOM(actor int32, msg []byte, session int64) {
...
func (self *PANDA) Old_MSGID_RECV_ACCOUNT_INFO(actor int32, data []byte, session int64) {
	...
	if sd == 1 {
		...
		sitdown.WriteUInt32(uint32(accId))
		core.CoreSend(0, self.owner.Id, sitdown.GetData(), 0)
		log.Debugf("玩家 %v %v 自动坐下", accId, name)
	}
}

func (self *PANDA) Old_MSGID_ENTER_GAME(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	accountId := pack.ReadUInt32()
	roomId := pack.ReadUInt32()
	if room.RoomMgr.IsMaintenance == true {
...
func (self *PANDA) Old_MSGID_MAINTENANCE_NOTICE(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	if session != 0 {
		log.Warnf("Error, 异常session:%v 处理消息编号:%v", session, pack.GetMsgID())
		return
...
func (self *PANDA) Old_MSGID_BACKSTAGE_CLOSE_ROOM(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
...
func (self *PANDA) Old_MSGID_BACKSTAGE_SET_BONUSPOOL(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
...
func (self *PANDA) Old_MSGID_GET_ONE_BONUSPOOL(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
...
func (self *PANDA) Old_MSGID_CHANGE_PLAYER_INFO(actor int32, msg []byte, session int64) {
...
func (self *PANDA) MSGID_HG_REENTER_OTHER_GAME(actor int32, msg []byte, session int64) {
