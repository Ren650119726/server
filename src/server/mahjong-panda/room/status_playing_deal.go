...
func (self *deal) Enter(now int64) {
	...
	duration := config.GetPublicConfig_Int64("PANDA_DEAL_TIME") // 持续时间 秒
	...
	self.owner.AddTimer(500, 1, func(dt int64) {
		...
		// 判断当前杠、胡
		if !self.deal_peng {
			self.all_hu = self.calcExtra(gamePlayer, 0, gamePlayer.cards.hand[gamePlayer.cards.last_index], true)
		...
		self.send.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_PUSH_CARD_NOTICE.UInt16())
		...
		// 如果手牌里还有定缺的牌，不能胡
		for _, card := range gamePlayer.cards.hand {
			if int8(card/10) == gamePlayer.decide_t {
				self.bhu = false
				break
			}
		}
		...
		if self.gangs == nil {
			...
			for i := len(self.gangs) - 1; i >= 0; i-- {
				card := gamePlayer.cards.hand[self.gangs[i]]
				t := card / 10
				if gamePlayer.decide_t == int8(t) {
...
func (self *deal) Tick(now int64) {
	if self.animation_ || self.delay2 {
		return
	}
	gamePlayer := self.seats[self.deal_player]
	if gamePlayer == nil {
	...
	if gamePlayer.trusteeship == 1 {
		if self.bhu {
			...
			if di := self.decided_MAX(gamePlayer); di != -1 {
				index = di
			}
			auto_push := packet.NewPacket(nil)
			auto_push.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_PUSH_CARD_REQ.UInt16())
...
func (self *deal) push_card(index int8, carIndex int, t int8) {
	...
	all_hu := self.calcExtra(gamePlayer, 0, gamePlayer.cards.hand[gamePlayer.cards.last_index], true)
	...
	send.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_PUSH_CARD_REQ.UInt16())
...
func (self *deal) Handle(actor int32, msg []byte, session int64) bool {
	...
	case protomsg.Old_MSGID_PANDA_GAME_PUSH_CARD_REQ.UInt16(): // 请求打牌
		self.Old_MSGID_PANDA_GAME_PUSH_CARD_REQ(actor, msg, session)
	case protomsg.Old_MSGID_PANDA_GAME_HU_REQ.UInt16(): // 请求胡
		self.Old_MSGID_PANDA_GAME_HU_REQ(actor, msg, session)
	case protomsg.Old_MSGID_PANDA_GAME_GANG_REQ.UInt16(): // 请求杠
...
// 过
func (self *deal) Old_MSGID_PANDA_GAME_GUO_REQ(actor int32, msg []byte, session int64) {
	if self.delay2 {
		log.Warnf("error 消息发早了 过")
		return
	}
	pack := packet.NewPacket(msg)
...
// 玩家请求打牌
func (self *deal) Old_MSGID_PANDA_GAME_PUSH_CARD_REQ(actor int32, msg []byte, session int64) {
	if self.delay2 {
		log.Warnf("消息发早了 打牌")
		return
	}
	...
	push_card := player.cards.hand[cardIndex]
	// 如果手牌里还有定缺的牌，不能胡
	exist := false
	...
	if int8(push_card/10) != player.decide_t && exist {
		log.Warnf("打出错误的牌，玩家手牌还有定缺的牌")
		return
	}
...
// 玩家胡牌
func (self *deal) Old_MSGID_PANDA_GAME_HU_REQ(actor int32, msg []byte, session int64) {
	if self.delay2 {
		log.Warnf("error 消息发早了 胡牌")
		return
	}
	pack := packet.NewPacket(msg)
	accid := pack.ReadUInt32()
	//hutype := 1 //pack.ReadUInt8() // 1胡   2 请胡
	...
	if gamePlayer.acc.AccountId != self.seats[self.master].acc.AccountId && len(gamePlayer.cards.gang) == 0 && gamePlayer.card_time == 1 {
		h = common.HU_DI
		self.all_hu[0].HuType = h
	}
	if gamePlayer.acc.AccountId != self.seats[self.master].acc.AccountId {
	...
	// 自摸胡牌积分逻辑
	total_money, packt := self.zimo(gamePlayer, self.all_hu)
...
// 玩家请求杠牌
func (self *deal) Old_MSGID_PANDA_GAME_GANG_REQ(actor int32, msg []byte, session int64) {
	if self.delay2 {
		log.Warnf("消息发早了 杠牌")
		return
	}
	...
	if int8(card/10) == gamePlayer.decide_t {
		log.Warnf("roomId:%v 玩家:%v 牌:%v 不能杠这张牌:%v, 定缺的牌:%v ", self.roomId, gamePlayer.acc.AccountId, gamePlayer.cards.String(), gangIndex, gamePlayer.decide_t)
		return
	}
