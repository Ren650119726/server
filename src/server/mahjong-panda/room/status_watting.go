...
func (self *watting) Enter(now int64) {
	...
	self.settle_pig = packet.NewPacket(nil)
	...
	// 玩家数据初始化 //////////////////////////////////////////////////
	timeout := utils.SecondTimeSince1970() + config.GetPublicConfig_Int64("PANDA_READY_TIME")
	bt := false
	for i, player := range self.seats {
		...
		if player.acc.Profit > 0 && int64(player.acc.Games) < config.GetPublicConfig_Int64("PANDA_REWARD_QUIT_COUNT") {
			player.safe_quit_timeout = utils.SecondTimeSince1970() + config.GetPublicConfig_Int64("PANDA_SAVE_QUIT_TIME")
		}
	...
	for _, v := range self.seats {
		if v != nil {
			...
			safe_time.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_UPDATE_SAFE_TIME.UInt16())
	...
	send.SetMsgID(protomsg.Old_MSGID_PANDA_GAME_WAITTING.UInt16())
	...
	self.timerID = self.owner.AddTimer(3000, -1, self.checkRobot)
...
func (self *watting) Handle(actor int32, msg []byte, session int64) bool {
	...
	case protomsg.Old_MSGID_PANDA_PREPARE.UInt16(): // 准备
		self.Old_MSGID_PANDA_PREPARE(actor, msg, session)
	default:
...
// 请求准备
func (self *Room) Old_MSGID_PANDA_PREPARE(actor int32, msg []byte, session int64) {
	pack := packet.NewPacket(msg)
	accountID := pack.ReadUInt32()

	send := packet.NewPacket(nil)
	send.SetMsgID(protomsg.Old_MSGID_PANDA_PREPARE.UInt16())
